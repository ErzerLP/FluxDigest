...
- **返回概览**：JSON 对象（非数组）；实际文章列表在 `items` 数组中，条目包含文章基础信息、来源信息、翻译结果、核心摘要、重要度等字段
...
- **返回概览**：JSON 对象（非数组）；`items` 数组中是每条 dossier 记录，含 dossier ID、关联文章 ID、译文标题、核心摘要、主题分类、推荐发布状态、发布状态等视图字段
...
### 常见错误含义（示例，仅供参考）
- **400**：通常代表请求体、query 参数或时间格式不合法；具体接口的 schema 合约和校验逻辑以 `api/openapi/openapi.yaml` 为准
- **401**：鉴权失败，例如 `POST /api/v1/jobs/daily-digest` 的 `X-API-Key` 无效，但不同端点可能有更细粒度的授权说明
- **500**：通常用于内部处理失败，例如配置更新、连通性测试或任务入队过程中报错；并非所有端点都统一返回此状态
- **503**：依赖未配置或相关服务不可用时常见的状态码；实际接口是否返回 503 需以 OpenAPI 合约和实现为准
