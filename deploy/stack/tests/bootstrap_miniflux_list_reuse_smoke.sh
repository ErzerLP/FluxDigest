...
if command -v python >/dev/null 2>&1 && python --version >/dev/null 2>&1; then
  ...
  echo "python/python3 is required for bootstrap_miniflux_list_reuse_smoke.sh" >&2
  exit 1
fi
    ...
    def do_GET(self):
        if self.path == "/v1/api-keys":
            body = json.dumps([
                {
                    "id": 1,
                    "description": "FluxDigest Installer",
                    "token": "listed-miniflux-token",
                }
            ]).encode()
            ...
            return
        if self.path == "/v1/me" and self.headers.get("X-Auth-Token") == "listed-miniflux-token":
...
HTTPServer(("127.0.0.1", 39201), Handler).serve_forever()
PY
...
echo "bootstrap_miniflux list reuse smoke passed"
