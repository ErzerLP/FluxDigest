...
if command -v python >/dev/null 2>&1 && python --version >/dev/null 2>&1; then
  ...
  echo "python/python3 is required for bootstrap_halo_setup_retry_smoke.sh" >&2
  exit 1
fi
...
class Handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        if self.path == "/apis/uc.api.security.halo.run/v1alpha1/personalaccesstokens":
            ...
            body = json.dumps({
                "metadata": {
                    "annotations": {
                        "security.halo.run/access-token": "retry-halo-access-token",
                    },
                },
            }).encode()
    ...
    def do_GET(self):
        if self.path == "/system/setup":
            state["setup_get_count"] += 1
            if state["setup_get_count"] <= 3:
                body = json.dumps({
                    "status": 500,
                    "title": "server error during bootstrap warmup",
                }).encode()
            ...
            body = (
                ...
                '<input type="hidden" name="_csrf" value="retry-csrf-token" />'
        ...
        if self.path.startswith("/apis/api.console.halo.run/v1alpha1/posts") and self.headers.get("Authorization") == "Bearer retry-halo-access-token":
...
HTTPServer(("127.0.0.1", 39322), Handler).serve_forever()
PY
...
export HALO_SITE_TITLE="FluxDigest"
export APP_PUBLISH_HALO_TOKEN=""

token="$(bootstrap_halo)"
[[ "${token}" == "retry-halo-access-token" ]]

echo "bootstrap_halo setup retry smoke passed"
