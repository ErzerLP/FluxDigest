...
export HTTP_PROXY="http://host.docker.internal:7890"
export HTTPS_PROXY="http://host.docker.internal:7890"
export no_proxy="corp.internal,localhost,127.0.0.1,::1,postgres,redis,miniflux,halo"
export NO_PROXY="${no_proxy}"
export BUILD_HTTP_PROXY="http://127.0.0.1:7890"
export BUILD_HTTPS_PROXY="http://127.0.0.1:7890"
...
grep -q "http_proxy=http://host.docker.internal:7890" "${STACK_DIR}/.env" || fail "运行时 http_proxy 未写入"
grep -q "NO_PROXY=corp.internal,localhost,127.0.0.1,::1,postgres,redis,miniflux,halo" "${STACK_DIR}/.env" || fail "NO_PROXY 未写入"
grep -q "no_proxy=corp.internal,localhost,127.0.0.1,::1,postgres,redis,miniflux,halo" "${STACK_DIR}/.env" || fail "no_proxy 未写入"
grep -q "BUILD_HTTP_PROXY=http://127.0.0.1:7890" "${STACK_DIR}/.env" || fail "BUILD_HTTP_PROXY 未写入"
grep -q "BUILD_HTTPS_PROXY=http://127.0.0.1:7890" "${STACK_DIR}/.env" || fail "BUILD_HTTPS_PROXY 未写入"
grep -q "BUILD_NO_PROXY=corp.internal,localhost,127.0.0.1,::1,postgres,redis,miniflux,halo" "${STACK_DIR}/.env" || fail "BUILD_NO_PROXY 未写入"
...
grep -q "HTTP_PROXY: \${BUILD_HTTP_PROXY}" "${STACK_DIR}/docker-compose.yml" || fail "compose 未透传构建 BUILD_HTTP_PROXY"
grep -q "NO_PROXY: \${BUILD_NO_PROXY}" "${STACK_DIR}/docker-compose.yml" || fail "compose 未透传构建 BUILD_NO_PROXY"
grep -q "GOPROXY: \${GOPROXY}" "${STACK_DIR}/docker-compose.yml" || fail "compose 未透传 Go 代理"
grep -q 'host.docker.internal:host-gateway' "${STACK_DIR}/docker-compose.yml" || fail "compose 未为容器注入 host.docker.internal"
